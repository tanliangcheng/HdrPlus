...
bool FastDetectCorner(int nYWin[][7], int nRatio, int nPtThre1, int nPtThre2, int nThre)
{
	...
	for (i = 0; i < 4; i++)
	{
		...
		if (N < nPtThre1 && P < nPtThre1)
			return false;
		nTotalN += N;
		nTotalP += P;
	}
	if (nTotalN < nPtThre2 && nTotalP < nPtThre2)
		return false;
	return true;
}
...
float ComputeHarrisCorner(unsigned char nYWin[][11], int x, int y, float fHarris_K)
{
