...
{
	if (!SetImageSize(nWidth, nHeight, 3))
		return false;
...
bool MultiUcharImage::GetAllChannelImage(SingleUcharImage *pOutImage)
{
	...
	if (nCh == 3)
	{
		...
		{
			if (!pOutImage[0].CreateImage(nWidth, nHeight))
				return false;
			if (!pOutImage[1].CreateImage(nWidth, nHeight))
				return false;
			if (!pOutImage[2].CreateImage(nWidth, nHeight))
				return false;
		}
		int nProcs = omp_get_num_procs();
#pragma omp parallel for num_threads(nProcs) schedule(dynamic, 16)
...
bool MultiUcharImage::ReplaceSingleChannelImage(SingleUcharImage *pInputImage)
{
	...
	{
		if (!pInputImage->CreateImage(nWidth, nHeight))
			return false;
	}
	int nProcs = omp_get_num_procs();
#pragma omp parallel for num_threads(nProcs) schedule(dynamic, 16)
...
bool MultiUcharImage::ReplaceAllChannelImage(SingleUcharImage *pInputImage)
{
	...
	if (nCh == 3)
	{
		...
		{
			if (!pInputImage[0].CreateImage(nWidth, nHeight))
				return false;
			if (!pInputImage[1].CreateImage(nWidth, nHeight))
				return false;
			if (!pInputImage[2].CreateImage(nWidth, nHeight))
				return false;
		}
		int nProcs = omp_get_num_procs();
#pragma omp parallel for num_threads(nProcs) schedule(dynamic, 16)
...
bool MultiUcharImage::SubtractEdgeImage(MultiUcharImage *pInImage, MultiShortImage *pOutImage)
{
	...
	if (nWidth1 < nWidth0 || nHeight1 < nHeight0)
		return false;
	if (!pOutImage->CreateImage(nWidth0, nHeight0, nCh))
		return false;
...
bool MultiUcharImage::GetMeanYVal(float &value)
{
	...
	if (!YUVImage.YUV444ToYUV420(&tmpYUV))
		return false;
...
#pragma omp parallel for num_threads(nThread) schedule(dynamic, 32)
	for (int y = 0; y < m_nHeight; y++)
	{
		...
		for (; x < m_nWidth; x++, pLine += 3)
		{
			...
			yuv[1] = (bgr[2] * 128 - bgr[1] * 85 - bgr[0] * 43) / 256;
			yuv[1] += 128;
			yuv[2] = (bgr[0] * 128 - bgr[1] * 107 - bgr[2] * 21) / 256;
			yuv[2] += 128;
...
#pragma omp parallel for num_threads(nThread) schedule(dynamic, 32)
	for (int y = 0; y < m_nHeight; y++)
	{
		...
		for (; x < m_nWidth; x++, pLine += 3)
		{
			...
			yuv[0] = (bgr[0] * 29 + bgr[1] * 150 + bgr[2] * 77 + 128) >> 8;
			yuv[1] = (bgr[0] * 128 - bgr[1] * 85 - bgr[2] * 43) / 256;
			yuv[1] += 128;
			yuv[2] = (bgr[2] * 128 - bgr[1] * 107 - bgr[0] * 21) / 256;
			yuv[2] += 128;
			for (int i = 0; i < 3; i++)
			{
				if (yuv[i] < 0)
...
bool MultiUcharImage::RGB2Y(SingleUcharImage *pOutImageYImage)
{
	...
	{
		if (!pOutImageYImage->CreateImage(nWidth, nHeight))
			return false;
	}
	int nThread = omp_get_num_procs();
#pragma omp parallel for num_threads(nThread) schedule(dynamic, 32)
...
bool MultiUcharImage::RGBToHue(MultiShortImage *pOutHueImage)
{
	...
	{
		if (!pOutHueImage->CreateImage(nWidth, nHeight, 1))
			return false;
	}
	int nThread = omp_get_num_procs();
#pragma omp parallel for num_threads(nThread) schedule(dynamic, 32)
...
{
	int y;
	if (!CreateImage(nOutWidth, nOutHeight))
		return false;
	if (nLeft == 0 && nRight == 0)
	{
		int nWidth = pInputImage->GetImageWidth();
