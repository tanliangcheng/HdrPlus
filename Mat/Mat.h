...
public:
	...
	inline void GetImagePixelBlock(int left, int right, int top, int bottom, T *oBuf)
	{
		if (left >= 0 && right < m_nWidth)
		{
			...
			{
				int ii = i < 0 ? 0 : i >= m_nHeight ? m_nHeight - 1
													: i;
		...
		else
		{
			...
			for (int i = top; i <= bottom; i++)
			{
				...
				if (m_nChannel == 1)
				{
					...
					{
						int jj = (j < 0) ? 0 : (j >= m_nWidth) ? m_nWidth - 1
															   : j;
						oBuf[0] = m_pImgData[(ii * m_nWidth + jj)];
						oBuf++;
					}
				}
				else
				{
					for (int j = left; j <= right; j++)
					{
						...
						{
							oBuf[0] = m_pImgData[(ii * m_nWidth + jj) * m_nChannel + n];
							oBuf++;
						}
					}
				}
...
#pragma omp parallel for schedule(dynamic, 32)
	...
	bool FillImageAround(CMat<T> *pOutImage, int padx, int pady)
	{
		...
		if (!pOutImage->SetImageSize(nNewWidth, nNewHeight, m_nChannel))
			return false;
		if (padx == 0 && pady == 0)
		{
...
#pragma omp parallel for schedule(dynamic, 32)
	...
	bool CopyImageRect(CMat<T> *pImage, int nLeft, int nTop, int nRight, int nBottom)
	{
		...
		if (!SetImageSize(nRight - nLeft, nBottom - nTop, nDim))
			return false;
#pragma omp parallel for
	...
	bool SetImageRect(CMat<T> *pImage, int nLeft, int nTop)
	{
		...
		if (nLeft + nW > m_nWidth || nTop + nH > m_nHeight)
			return false;
		if (nLeft < 0 || nTop < 0)
			return false;
...
#pragma omp parallel for
	...
	{
		if (m_pImgData == NULL || m_nHeight < 3)
			return false;
		if (ch < 0)
			ch = 0;
		if (ch >= m_nChannel)
...
class CArray3D
{
	...
	public:
		...
		inline CArray1D &operator[](long elem) const
		{
			return p[elem];
		}
...
public:
	...
	inline CArray2D &operator[](long elem) const
	{
		return p[elem];
	}
