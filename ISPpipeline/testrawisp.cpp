...
bool LoadMetaDataTxtFile(char *pFileName)
{
	...
	if (fp == NULL)
		return false;
	if (fscanf(fp, "wb rgb gain(%f,%f,%f)\n", &m_fRGain, &m_fGGain, &m_fBGain) != 3)
		return false;
	printf("AWBGain=[%f,%f,%f]\n", m_fRGain, m_fGGain, m_fBGain);
	if (fscanf(fp, "[%f,%f,%f,\n", &m_f[0][0], &m_f[0][1], &m_f[0][2]) != 3)
		return false;
	if (fscanf(fp, "[%f,%f,%f,\n", &m_f[1][0], &m_f[1][1], &m_f[1][2]) != 3)
		return false;
	if (fscanf(fp, "[%f,%f,%f,\n", &m_f[2][0], &m_f[2][1], &m_f[2][2]) != 3)
		return false;
...
int main(int argc, char *argv[])
{
	...
	if (!m_HDRPlus_Forward.LoadMultiConfigFile("./config/weight.param"))
	{
